...
// fromTemplate 是 PromptTemplate 的静态工厂方法（快捷方式），
// 自动从模板字符串中提取 {变量名} 作为 inputVariables，
// 返回一个 PromptTemplate 实例对象 naiveTemplate。
...
// format 是 PromptTemplate 的实例方法，由 naiveTemplate 这个实例对象调用，
// 将模板中 {变量名} 替换为实际值，返回填充后的 prompt 字符串。
...
// 同一个 naiveTemplate 实例可以多次调用 format，每次传入不同的数据生成不同的 prompt。
